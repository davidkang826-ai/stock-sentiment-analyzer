...
import { getChannelIdFromUrl, getChannelInfoFromVideo, getChannelVideos, getLatestVideos, getSingleVideoInfo, getTranscript } from "./youtube.js";
...
// ── GET /api/channel-from-url?url=... ────────────────────────────────────────
app.get("/api/channel-from-url", async (req, res) => {
  const { url } = req.query;
  if (!url || typeof url !== "string") { res.status(400).json({ error: "url required" }); return; }
  try {
    const info = await getChannelIdFromUrl(url);
    res.json(info);
  } catch (e: any) {
...
// ── POST /api/analyze-channel — latest 10 videos SSE stream ──────────────────
app.post("/api/analyze-channel", async (req, res) => {
  ...
  try {
    emit({ type: "progress", message: "Fetching latest 10 videos..." });
    ...
    for (const video of videos) {
      ...
      if (analysis.tickers.length > 0) {
        emit({ type: "progress", message: `📈 Found ${analysis.tickers.length} ticker(s): ${analysis.tickers.map((t) => t.ticker).join(", ")}` });
